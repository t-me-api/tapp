...
class ChatType(Filter):
    def __init__(self, chat_type: Union[enums.ChatType, Sequence[enums.ChatType]]) -> None:
    ...
    def find_chat_type(
        ...
        if isinstance(update, types.InlineQuery):
            ...
            isinstance(update, types.Message)
            or isinstance(update, types.ChatMemberUpdated)
            or isinstance(update, types.ChatJoinRequest)
        ):
